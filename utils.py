...
def load_model(critic: Discriminator, G: Generator,strict=False):
    try:
...
def save_model(D: Discriminator, G: Generator):
...
def sample_and_exhibit(winname,winname2,generator=None,device=None,samples=None):
    ...
    if generator is not None:
        # generator.eval()
        z = torch.randn([64,*p.LATENT_SINGLE_SHAPE], device=device)
        sample = generator(z).cpu()
        sample = einops.rearrange(
            sample, '(b_x b_y) c h w -> (b_y h) (b_x w) c', b_x=8).numpy()
